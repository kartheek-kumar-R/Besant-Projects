...
create table customer_income_status (SELECT *, if(Applicantincome >15000,"Grade A",
if(Applicantincome >9000,"Grade B",
if(Applicantincome > 5000,"Middle Class","Low Class"))) as Customer_Class,
if(Applicantincome < 5000 and Property_Area ="RURAL",3,
if(Applicantincome < 5000 and Property_Area ="SEMIRURAL",3.5,
if(Applicantincome < 5000 and Property_Area ="URBAN",5,
if(Applicantincome < 5000 and Property_Area ="SEMIURBAN",2.5,7))))as Customer_Interest
...
(loan_id varchar(60),customer_id varchar(60),loan_amount varchar(50),loan_amount_term int,cibil_score int);

create table loan_status_remarks(loan_id varchar(60),loan_amount varchar(50),cibil_score int,cibil_score_status varchar(60));

delimiter //
create trigger loan_verify before insert
...
delimiter //
create trigger verify_cibil_score after insert 
on loan_status for each row
begin
if new.cibil_score > 900 
then insert into loan_status_remarks(loan_id,loan_amount,cibil_score,cibil_score_status)
...
select * from loan_status;
select * from loan_status_remarks;

drop table loan_status_remarks;
drop table loan_status;
drop trigger loan_verify;
drop trigger verify_cibil_score;

delete from loan_status_remarks where cibil_score_status = 'reject customers';
delete from loan_status_remarks where loan_amount ='loan still processing';
Alter table loan_status_remarks modify loan_amount int;

create table loan_cibil_score_status
( select loan_status.customer_id,loan_status.loan_id,loan_status_remarks.loan_amount,loan_status.loan_amount_term,
loan_status.cibil_score,loan_status_remarks.cibil_score_status from loan_status
join loan_status_remarks on loan_status.loan_id = loan_status_remarks.loan_id);

select * from loan_cibil_score_status;
select * from loan_status_remarks;
drop table loan_cibil_score_status;
...
c.customer_class,concat(c.customer_interest,'%') as customer_interest,l.loan_amount,
l.cibil_score,l.cibil_score_status,round((c.customer_interest * l.loan_amount)/100,2) as monthly_interest,
round((c.customer_interest * l.loan_amount*12)/100,2) as annual_interest from CUSTOMER_INCOME_STATUS c 
inner join loan_cibil_score_status l on c.loan_id = l.loan_id);
...
begin
select l.loan_id,l.customer_id,l.applicantincome,l.property_area,l.coapplicantincome,
l.customer_class,l.loan_status,l.customer_interest,l.cibil_score,l.cibil_score_status,
l.monthly_interest,l.annual_interest,c.customer_name,c.gender,c.age,
c.married,c.education,c.self_employed,s.postal_code,s.segment,s.state,r.region,r.region_id 
from customer_interest_analysis l inner join customer_det c on l.loan_id = c.loan_id
inner join country_state s on c.customer_id = s.customer_id
inner join region_info r on s.region_id = r.region_id;

select c.*from customer_det c left join customer_interest_analysis l
 on c.customer_id = l.customer_id where l.customer_id is null;
 
 select c.*, r.region,r.region_id from country_state c right join
region_info r on c.region_id = r.region_id ;
 
 select * from customer_interest_analysis where cibil_score_status ='high cibil score';
